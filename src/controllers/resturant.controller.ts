...
export class ResturantController {
    ...
    async registerResturant(@Body() regReq: IRegisterResturantReq): Promise<IRegisterResturantRes> {
        const newResturant: IResturant = await new ResturantRepo().registerResturant(<IResturant>(regReq));
    ...
    async deleteResturant(@Body() delReq: IDeleteResturantReq) {
        return await new ResturantRepo().deleteResturant(delReq._id)
    }

    @Post('getResturant')
    async getResturant(@Body() getReq: IGetResturantReq): Promise<IRegisterResturantRes> {
        const newResturant: IResturant = <any> await new ResturantRepo().getResturant(getReq._id);
        if(newResturant === null) throw new CustomError(404, 'Resturant not found');
        return <IRegisterResturantRes> newResturant
    }

    @Post('checkNearByResturants')
    async checkNearByResturants(@Body() location: ICheckNearbyResturantReq): Promise<ICheckNearbyResturantRes[]> {
        const resturants: IResturant[] = await new ResturantRepo().checkNearByResturants();
        let nearResturants: any [] = [];
        resturants.map(resturant => {
            const distance = geolib.getDistance({ latitude: resturant.location.latitude, longitude: resturant.location.longitude },
